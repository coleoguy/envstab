...
GetRecomb <- function(chrom.num,
  ...
  for(i in 1:nrow(gen.struct)){
    # first column is starting locus of each chromosome
    gen.struct[i, 1] <- chrom.size*(i-1)+1
    #second column is ending locus of each chromosome
  ...
  for(i in 1:chrom.num){
    ...
    samp.range <- (gen.struct[i, 1]):(gen.struct[i, 2]-1)
    if(length(samp.range)>1){
      rspots[i] <- sample(samp.range, 1)
    }else{
      rspots[i] <- samp.range
    }
  }
...
Mutate <- function(population, mut.rate, loci, popsize){
...
simulate <- function(loci, chrom.num, popsize, generations, 
  ...
  for(j in 1:generations){
    ...
    # mutations
    population <- Mutate(population, mut.rate, loci, popsize)
